...
class MyProcess extends Thread {
    ...
    public static int battery =1500;
    ...
    public void run() {
        if (this.character ==0) { // the thread is a costumer.
            ...
            for (int i =0; i <100; i++) {
                if (battery <=700 && currentPassanger ==0) {
                    System.out.println("Bus is going to power station and charging");
                    try{
                        Thread.sleep(randInt(1, 2)*100);
                    }catch (InterruptedException e) {
                        e.printStackTrace();
                    }
                    battery =1500;
                    continue;
                }else if(battery <=700 && currentPassanger !=0){
                    System.out.println("Bus is at station: "+i%6);
                    currentTerminal = i%6;
                    System.out.println("Bus is leaving station: "+currentTerminal+" with "+currentPassanger+" passangers on bus and the battery is: "+battery);
                    int travelingT =randInt(1, 2)*100;
                    try{
                        Thread.sleep(travelingT);
                    }catch (InterruptedException e) {
                        e.printStackTrace();
                    }
                    battery -= travelingT;
